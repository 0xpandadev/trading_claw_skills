...
一般的な「投資アドバイス系プロンプト」は再現性が低く、改善も難しいです。  
このリポジトリは、分析プロセスをスキルとして構造化し、次を実現するために作っています。
...
- `tradermonty/claude-trading-skills` 由来の **52スキル**（中身を保持）
- カスタム拡張 **2スキル**
  - `tenbagger-screener`
...
- `pead-screener`
- `pair-trade-screener`
- `tenbagger-screener`（拡張）
- `buffett-value-evaluator`（拡張）
